...
std::function<void(PlyLoader&)> PlyLoader::ins::ply_t::process(file_type tv)
{
  auto upload = [](auto&& mesh, auto&& buf)
  {
    return [mesh = std::move(mesh), buf = std::move(buf)](PlyLoader& o) mutable
    {
  ...
  QVKRT::float_vec buf;
  if (check_file_extension(tv.filename, "ply"))
  {
