...
void RayTracer::init(VkPhysicalDevice physDev, VkDevice dev, QVulkanFunctions *f, QVulkanDeviceFunctions *df)
{
...
VkImageLayout RayTracer::doIt(QVulkanInstance *inst,
...
RayTracer::Buffer RayTracer::createHostVisibleBuffer(int usage, VkPhysicalDevice physDev, VkDevice dev, QVulkanFunctions *f, QVulkanDeviceFunctions *df, uint32_t size)
{
...
void RayTracer::updateHostData(const Buffer &b, VkDevice dev, QVulkanDeviceFunctions *df, const void *data, size_t dataLen)
{
    void *p = nullptr;
...
void RayTracer::freeBuffer(const Buffer &b, VkDevice dev, QVulkanDeviceFunctions *df)
{
    df->vkDestroyBuffer(dev, b.buf, nullptr);
    df->vkFreeMemory(dev, b.mem, nullptr);
}

VkDeviceAddress RayTracer::getBufferDeviceAddress(VkDevice dev, const Buffer &b)
{
